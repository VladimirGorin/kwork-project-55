...
class User:
    ...
    def export_stick_data(self):
        ...
        if "Приклеить" in confirm_button.text:
            ...
            try:
                ...
                self.browser.find_element(By.XPATH, path.SPINS_CONFRIM_BUTTON_XPATH).click()
                time.sleep(t.sleep)
                parsed_url = urlparse(self.browser.current_url)

                stick_price = int(parse_qs(parsed_url.query).get("stickPrice")[0])

                updated_url = self.replace_stick_price(old_url, stick_price)
        ...
        elif "Сохранить" in confirm_button.text:
            ...
            raise Exception("Кнопка для подтверждения не найдена")
    ...
    def launch_ad(self, ad_link, user_info):
        try:
            ...
            self.error_message(
                message=f"При переходе на товар: {e}", chat_id=self.chat_id)
